...
type ParagraphField = { id: string; text: string };
...
const createParagraph = (text = ""): ParagraphField => ({
  id: createId(),
  text,
});
  ...
  return (
    <div ref={setNodeRef} style={style} className="rounded-2xl border border-border bg-surface p-4">
      <div className="mb-3 flex items-center justify-between">
        <span className="text-xs font-semibold text-text-muted">Parágrafo {index + 1}</span>
...
export function NewChapterWizard({ bookId }: { bookId: string }) {
  ...
  const saveBulkMode = async () => {
    ...
    setSaving(true);
    try {
      await chaptersService.bulkInsert(chapterId, bulkText);
      handlePostSave();
    } catch (error) {
  ...
  const renderStep = () => {
    ...
    const preview = segmentText(bulkText);

    return (
      <div className="space-y-6">
        <Textarea
          label="Texto completo"
          placeholder="Cole o texto completo do capítulo. Parágrafos serão identificados por linhas em branco."
          value={bulkText}
          onChange={(event) => setBulkText(event.target.value)}
        />
        <div className="rounded-3xl border border-border bg-surface p-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-text-muted">
            Preview ({preview.length} parágrafos)
          </p>
          <div className="mt-4 space-y-3 text-sm text-text">
            {preview.length === 0 ? (
              <p className="text-sm text-text-muted">Cole o texto para visualizar a segmentação.</p>
            ) : (
              preview.map((paragraph, index) => (
                <div key={`preview-${index}`} className="rounded-2xl bg-surface-muted px-4 py-3">
                  <p className="text-xs text-text-muted">Parágrafo {index + 1}</p>
